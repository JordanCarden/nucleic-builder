...
class martini22nucleic:
    def __init__(self):
        ...
        self.dnaBbBondDictC = dict(list(zip(self.dna_con['bond'],self.dna_bb['bond'])))
        # Dictionary for the connectivities and parameters of angles between DNA backbone beads
        self.dnaBbAngleDictC = dict(list(zip(self.dna_con['angle'],self.dna_bb['angle'])))
        # Dictionary for the connectivities and parameters of dihedrals between DNA backbone beads
        self.dnaBbDihDictC = dict(list(zip(self.dna_con['dih'],self.dna_bb['dih'])))
        # Dictionary for exclusions for DNA backbone beads
        self.dnaBbExclDictC = dict(list(zip(self.dna_con['excl'],self.dna_bb['excl'])))
        # Dictionary for pairs for DNA backbone beads
        self.dnaBbPairDictC = dict(list(zip(self.dna_con['pair'],self.dna_bb['pair'])))

        ## RNA DICTIONARIES ##
        # Dictionary for the connectivities and parameters of bonds between rna backbone beads
        self.rnaBbBondDictC = dict(list(zip(self.rna_con['bond'],self.rna_bb['bond'])))
        # Dictionary for the connectivities and parameters of angles between rna backbone beads
        self.rnaBbAngleDictC = dict(list(zip(self.rna_con['angle'],self.rna_bb['angle'])))
        # Dictionary for the connectivities and parameters of dihedrals between rna backbone beads
        self.rnaBbDihDictC = dict(list(zip(self.rna_con['dih'],self.rna_bb['dih'])))
        # Dictionary for exclusions for rna backbone beads
        self.rnaBbExclDictC = dict(list(zip(self.rna_con['excl'],self.rna_bb['excl'])))
        # Dictionary for pairs for rna backbone beads
        self.rnaBbPairDictC = dict(list(zip(self.rna_con['pair'],self.rna_bb['pair'])))
    ...
    def bbGetBond(self,r,ca,ss):
        ...
        if r[0] in dnares3:
            return ca in list(self.dnaBbBondDictC.keys()) and self.dnaBbBondDictC[ca] or None
        elif r[0] in rnares3:
            return ca in list(self.rnaBbBondDictC.keys()) and self.rnaBbBondDictC[ca] or None
        else:
            sys.exit("This script supports only DNA or RNA.")
    
    def bbGetAngle(self,r,ca,ss):
        if r[0] in dnares3:
            return ca in list(self.dnaBbAngleDictC.keys()) and self.dnaBbAngleDictC[ca] or None
        elif r[0] in rnares3:
            return ca in list(self.rnaBbAngleDictC.keys()) and self.rnaBbAngleDictC[ca] or None
        else:
            sys.exit("This script supports only DNA or RNA.")

    def bbGetExclusion(self,r,ca,ss):
        if r[0] in dnares3:
            return ca in list(self.dnaBbExclDictC.keys()) and ' ' or None
        elif r[0] in rnares3:
            return ca in list(self.rnaBbExclDictC.keys()) and ' ' or None
        else:
            sys.exit("This script supports only DNA or RNA.")

    def bbGetPair(self,r,ca,ss):
        if r[0] in dnares3:
            return ca in list(self.dnaBbPairDictC.keys()) and ' ' or None
        elif r[0] in rnares3:
            return ca in list(self.rnaBbPairDictC.keys()) and ' ' or None
        else:
            sys.exit("This script supports only DNA or RNA.")
    ...
    def bbGetDihedral(self,r,ca,ss):
        ...
        if r[0] in dnares3:
            return ca in list(self.dnaBbDihDictC.keys()) and self.dnaBbDihDictC[ca] or None
        elif r[0] in rnares3:
            return ca in list(self.rnaBbDihDictC.keys()) and self.rnaBbDihDictC[ca] or None
        else:
            sys.exit("This script supports only DNA or RNA.")
...
class Chain:
    ...
    def __getitem__(self,other):
        if isinstance(other, slice):
            ...
            # Python 2 called __getslice__(i, sys.maxint) for an open-ended
            # slice. Preserve that behavior: DNA's legacy ``seq`` attribute
            # stores two characters (dA/dC/dG/dT) per residue.
            stop = sys.maxsize if other.stop is None else other.stop
