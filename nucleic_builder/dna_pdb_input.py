"""Strict experimental DNA PDB validation and normalization.
...
CANONICAL_RESIDUES = frozenset({"A", "C", "G", "T"})
...
TERMINAL_ALIASES = {
    **{f"D{base}5": (base, "5") for base in CANONICAL_RESIDUES},
    **{f"D{base}3": (base, "3") for base in CANONICAL_RESIDUES},
...
ACCEPTED_RESIDUES = {
    ...
    **{f"D{base}": (base, None) for base in CANONICAL_RESIDUES},
    **TERMINAL_ALIASES,
}

# The upstream mapping aliases these modified nucleotides to canonical bases.
# Phase 2B rejects them explicitly: aliasing is not parameterization.
...
# Ordinary crystallographic waters are not part of the requested DNA molecule.
...
_SUGAR_HEAVY_ATOMS = frozenset({"C5'", "C4'", "O4'", "C3'", "C1'", "C2'"})
_BASE_HEAVY_ATOMS = {
...
class InputSummary:
    ...
    def expected_charge(self) -> float:
        # The DNA-alpha mapping omits the 5' phosphate bead on each chain.
        ...
        for position, residue in enumerate(chain_residues):
            ...
            if "O2'" in present:
                raise InputValidationError(
                    "RNA ribose O2' atom found in explicitly selected DNA at "
                    f"chain {_chain_label(residue.chain_id)} residue "
                    f"{residue.source_label}; DNA input must use deoxyribose"
                )
...
def prepare_dna_input_pdb(source: Path, destination: Path) -> InputSummary:
    """Validate *source* and write the normalized DNA-only PDB to *destination*.
    ...
    for line_number, raw_line in enumerate(lines, start=1):
        ...
        if record_name == "MODEL":
            ...
            if explicit_model_count > 1:
                raise InputValidationError(
                    "Phase 2B accepts exactly one PDB model; multiple MODEL records were found"
                )
            continue
        ...
        if starts_segment:
            ...
            if current_segment_index >= len(_NORMALIZED_CHAIN_IDS):
                raise InputValidationError(
                    f"Phase 2B supports at most {len(_NORMALIZED_CHAIN_IDS)} PDB chain segments"
                )
    ...
    if modified_records:
        raise InputValidationError(
            "Modified DNA nucleotides are not supported in Phase 2B and will not be mapped to "
    ...
    if unsupported_records:
        raise InputValidationError(
            ...
            f"{_format_locations(unsupported_records)}. Accepted DNA residue names are "
            "DA, DC, DG, DT (or A, C, G, T) and verified 3'/5' terminal variants."
        )
    if not coordinate_records:
        raise InputValidationError("Input PDB contains no canonical DNA atom records")
    ...
    if summary.residue_count > 9999:
        raise InputValidationError("Phase 2B PDB/GRO output supports at most 9,999 residues")
